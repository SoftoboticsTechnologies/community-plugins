...
describe('ImportWriterService.commit', () => {
    it('creates one product and one variant for a single-variant row', async () => {
        const deps = makeDeps();
        const writer = new ImportWriterService(deps.productService as any, deps.productVariantService as any, deps.assetImportService as any, deps.productOptionGroupService as any, deps.productOptionService as any);
